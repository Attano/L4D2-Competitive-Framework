...
{
	SM_hEnable = CreateConVar("SM_enable", "1", "L4D2 Custom Scoring - Enable/Disable", FCVAR_PLUGIN);
	HookConVarChange(SM_hEnable, SM_ConVarChanged_Enable);
	
	SM_hHBRatio = CreateConVar("SM_healthbonusratio", "2.0", "L4D2 Custom Scoring - Healthbonus Multiplier", FCVAR_PLUGIN, true, 0.25, true, 5.0);
	HookConVarChange(SM_hHBRatio, SM_CVChanged_HealthBonusRatio);
	
	SM_hSurvivalBonusRatio = CreateConVar("SM_survivalbonusratio", "0.0", "Ratio to be used for a static survival bonus against Map distance. 25% == 100 points maximum health bonus on a 400 distance map", FCVAR_PLUGIN);
	HookConVarChange(SM_hSurvivalBonusRatio, SM_CVChanged_SurvivalBonusRatio);
	
	SM_hTempMulti0 = CreateConVar("SM_tempmulti_incap_0", "0.30625", "L4D2 Custom Scoring - How important temp health is on survivors who have had no incaps", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(SM_hTempMulti0, SM_ConVarChanged_TempMulti0);
	
	SM_hTempMulti1 = CreateConVar("SM_tempmulti_incap_1", "0.17500", "L4D2 Custom Scoring - How important temp health is on survivors who have had one incap", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	HookConVarChange(SM_hTempMulti1, SM_ConVarChanged_TempMulti1);
	
	SM_hTempMulti2 = CreateConVar("SM_tempmulti_incap_2", "0.10000", "L4D2 Custom Scoring - How important temp health is on survivors who have had two incaps (black and white)", FCVAR_PLUGIN, true, 0.0, true, 1.0);
	...
	SM_hMapMulti = CreateConVar("SM_mapmulti", "1", "L4D2 Custom Scoring - Increases Healthbonus Max to Distance Max", FCVAR_PLUGIN);
	
	SM_hCustomMaxDistance = CreateConVar("SM_custommaxdistance", "0", "L4D2 Custom Scoring - Custom max distance from config", FCVAR_PLUGIN);
